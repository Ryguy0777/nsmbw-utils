...
CREATE_STATE(daEnKuribon_c, Dizzy);
CREATE_STATE(daEnKuribon_c, Carry);
CREATE_STATE(daEnKuribon_c, Throw);
CREATE_STATE(daEnKuribon_c, Kick);
CREATE_STATE(daEnKuribon_c, Recover);

dActor_c *daEnKuribon_c::build() {
	void *buffer = AllocFromGameHeap1(sizeof(daEnKuribon_c));
	return new(buffer) daEnKuribon_c;
}

const char* GaloombaArcList[] = {
    "kuribon",
    NULL
};
...
void daEnKuribon_c::itemPickedUp() {
...
void daEnKuribon_c::spriteCollision(ActivePhysics *apThis, ActivePhysics *apOther) {
    ...
    if (currentState != &StateID_Walk) {
        ...
        return daEnKuribo_c::spriteCollision(apThis, apOther);
    }
}
...
void daEnKuribon_c::playerCollision(ActivePhysics *apThis, ActivePhysics *apOther) {
...
void daEnKuribon_c::yoshiCollision(ActivePhysics *apThis, ActivePhysics *apOther) {
...
void daEnKuribon_c::dieOther_Begin() {
    return dEn_c::dieOther_Begin();
}

void daEnKuribon_c::dieOther_Execute() {
	return dEn_c::dieOther_Execute();
}

void daEnKuribon_c::dieOther_End() {
...
void daEnKuribon_c::reactFumiProc(dStageActor_c* killingActor) {
    doStateChange(&StateID_Dizzy);
    return;
}

void daEnKuribon_c::loadModel() {
...
void daEnKuribon_c::playWalkAnim() {
    nw4r::g3d::ResAnmChr resAnmChr = resFile.GetResAnmChr("walk");
...
s8 someByteArray[2] = {1, 0xFF};
//sets the collider when we carry
void daEnKuribon_c::doCarryCollider() {
    ...
    _24.y = _48.y;
    _24.z = _54.z;
    _30.x = _24.x + someByteArray[direction] * 16.0;
    _30.y = _48.y;
    _30.z = _54.z;
...
void daEnKuribon_c::killWhenCarried(dAcPy_c *player) {
    u8 *playerNum = player->_vf6C();
    deathInfo.isDead = 1;
...
s16 dizzyAngles[2] = {0xA000, 0x6000};

void daEnKuribon_c::beginState_Dizzy() {
    dStateBase_c *prevState = acState.getPreviousState();
...
void daEnKuribon_c::endState_Dizzy() { }
...
void daEnKuribon_c::beginState_Carry() {
...
void daEnKuribon_c::executeState_Carry() {
    ...
    if ((_12C & 1) == 0) {
        ...
        rot.y = dizzyAngles[carryDirection];
        ...
        if ((collMgr.outputMaybe & 0x15 << (actorDirection & 0x3f)) == 0) {
            pos.x = pos.x + someByteArray[actorDirection] * 6.0;
        }
        if ((_12C & 2) == 0) {
            doStateChange(&StateID_Dizzy);
        } else {
...
void daEnKuribon_c::endState_Carry() {
    dAcPy_c *player = dAcPy_c::findByID(carriedByPlayerID);
...
void daEnKuribon_c::beginState_Throw() {
...
void daEnKuribon_c::endState_Throw() {
    _134 = 0;
    x_speed_inc = 0.0;
    max_speed.x = 0.0;
    aPhysics.info.category2 = 0;
...
void daEnKuribon_c::endState_Kick() {
    _134 = 0;
    aPhysics.info.category2 = 0;
}

void daEnKuribon_c::beginState_Recover() {
...
void daEnKuribon_c::endState_Recover() { }