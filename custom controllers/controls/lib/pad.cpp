...
extern "C" void OSRegisterShutdownFunction(OSResetFunctionInfo* info);
...
extern "C" void OSSetWirelessID(s32, u16);
...
inline static void DoReset(void) {
  u32 chanBit;
  PADResetChan = cntlzw(PADResetBits);
  if (PADResetChan == 32)
    return;
...
void PAD_SamplingHandler(s16 interrupt, OSContext2* context) {
